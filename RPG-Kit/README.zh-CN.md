...
- **正向（需求 → RPG → 代码）：** 通过由 AI 编码智能体驱动的多阶段流水线，将高层需求转换为经过测试、结构化的代码仓库。[快速开始见这里](#quick-start-new-repository)
- **反向（代码 → RPG）：** 将现有代码库编码为 RPG 图，用于 AI 辅助搜索、探索和增量更新。[快速开始见这里](#quick-start-existing-repository)
- **外科式编辑（指令 → RPG + 代码）：** 应用有针对性的修改，同时保持代码、RPG 和依赖图同步。[快速开始见这里](#quick-start-existing-repository)
